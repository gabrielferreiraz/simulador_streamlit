...
def initialize_session_state():
    ...
    # Valores padrão para um formulário limpo
    defaults = {
        'cliente_nome': "",
        'valor_credito': None,
        'prazo_meses': None,
    ...
    if last_sim:
        # Se houver uma última simulação, use seus dados, caso contrário, use os padrões.
        st.session_state.simulation_inputs = {
            key: last_sim.get(key, defaults[key]) for key in defaults
        }
        # Ajustes específicos para os dados do banco
        st.session_state.simulation_inputs['cliente_nome'] = last_sim.get('nome_cliente', defaults['cliente_nome'])
        if 'taxa_administracao_total' in last_sim and last_sim['taxa_administracao_total'] is not None:
            st.session_state.simulation_inputs['taxa_administracao_total'] *= 100
        if 'percentual_lance_ofertado' in last_sim and last_sim['percentual_lance_ofertado'] is not None:
            st.session_state.simulation_inputs['percentual_lance_ofertado'] *= 100
        if 'percentual_lance_embutido' in last_sim and last_sim['percentual_lance_embutido'] is not None:
            st.session_state.simulation_inputs['percentual_lance_embutido'] *= 100
...
def show():
    ...
    with st.container(border=True):
        ...
        with st.expander("Parâmetros da Simulação", expanded=True):
            ...
            st.session_state.simulation_inputs['opcao_plano_light'] = col1.selectbox("Plano Light", options=list(config.PLANO_LIGHT_OPTIONS.keys()), format_func=lambda x: config.PLANO_LIGHT_OPTIONS[x], index=list(config.PLANO_LIGHT_OPTIONS.keys()).index(st.session_state.simulation_inputs.get('opcao_plano_light', 1)))
            st.session_state.simulation_inputs['opcao_seguro_prestamista'] = col2.selectbox("Seguro Prestamista", options=list(config.SEGURO_PRESTAMISTA_OPTIONS.keys()), format_func=lambda x: config.SEGURO_PRESTAMISTA_OPTIONS[x], index=list(config.SEGURO_PRESTAMISTA_OPTIONS.keys()).index(st.session_state.simulation_inputs.get('opcao_seguro_prestamista', 0)))
            ...
            col1, col2 = st.columns(2)
            st.session_state.simulation_inputs['opcao_diluir_lance'] = col1.radio("Diluir Lance?", options=list(config.DILUIR_LANCE_OPTIONS.keys()), format_func=lambda x: config.DILUIR_LANCE_OPTIONS[x], index=list(config.DILUIR_LANCE_OPTIONS.keys()).index(st.session_state.simulation_inputs.get('opcao_diluir_lance', 1)))
    ...
    if st.session_state.simulation_results and st.session_state.simulation_results["status"] == "success":
        ...
        # ... (a exibição dos resultados permanece a mesma)
        ...
        if pdf_data:
            safe_client_name = sanitize_filename(st.session_state.simulation_inputs.get('cliente_nome', 'cliente'))
            ...
            st.error("Ocorreu um erro ao gerar o PDF. A equipe de TI foi notificada. Por favor, tente novamente mais tarde.")
